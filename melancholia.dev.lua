...
local function logsclr(color)
    local output = ''
    output = output .. ('\a%02x%02x%02x%02x'):format(color.r, color.g, color.b, color.a or 255)
    return output
end
...
local function aimack(shot)
    ...
    if shot.state == nil then
        if menu.hitlogs:get(2) then
            print_raw(("\aFFFFFFFF["..hitcolor.."melancholia\aFFFFFFFF] Registered shot in "..shot.target:get_name().."\'s "..hitcolor..hitgroup_str[shot.hitgroup].."\aFFFFFFFF for "..hitcolor..shot.damage.."\aFFFFFFFF("..hitcolor..shot.wanted_damage.."\aFFFFFFFF) (hitchance: "..hitcolor..shot.hitchance.."% \aFFFFFFFF| history(Δ): "..hitcolor..shot.backtrack.." \aFFFFFFFF| flags: "..hitcolor..(refs.double_tap:get() and 1 or 0)..(refs.hide_shots:get() and 1 or 0).."\aFFFFFFFF)"))
        end
        ...
        if menu.hitlogs:get(2) then
            print_raw(("\aFFFFFFFF["..misscolor.."melancholia\aFFFFFFFF] Missed shot in "..shot.target:get_name().."\'s "..misscolor..hitgroup_str[shot.wanted_hitgroup].."\aFFFFFFFF due to "..misscolor..shot.state.." \aFFFFFFFF(hitchance: "..misscolor..shot.hitchance.."% \aFFFFFFFF| history(Δ): "..misscolor..shot.backtrack.." \aFFFFFFFF| flags: "..misscolor..(refs.double_tap:get() and 1 or 0)..(refs.hide_shots:get() and 1 or 0).."\aFFFFFFFF)"))
        end
        table.insert(miscellaneous.stuff,{text2 = {
            name = '',
            hit = 'Missed ',
